...
import argparse
from utils.data_process import tokenize, resize_frame
import metaworld
...
def visualize_results(model, env_name, camera="topview",
    ...
    env = gym.make("Meta-World/MT1", env_name=env_name, seed=seed,
                   render_mode="rgb_array", camera_name="gripperPOV")
    top_view_env = gym.make("Meta-World/MT1", env_name=env_name, seed=seed,
    ...
    frames  = []
    success = False
    tokens  = tokenize("pick the red ball").unsqueeze(0).to(device)  

    for step in range(max_steps):
        frame = env.render()
        frames.append(top_view_env.render())

        img    = resize_frame(frame)   
        img = torch.FloatTensor(img).permute(2, 0, 1) / 255.0
        img = img.unsqueeze(0).to(device) 
        state  = torch.FloatTensor(obs[:4]).unsqueeze(0).to(device)      
        action = sample_action(model, img, tokens, state,
                               action_dim=4, steps=100, device=device)   

        obs, reward, terminated, truncated, info = env.step(action.cpu().numpy())
        top_view_env.step(action.cpu().numpy())
